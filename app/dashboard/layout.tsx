...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!dbUser?.isAdmin && !roleAccess["dashboard"]) redirect("/knowledge");
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <AppHeader session={effectiveSession} isAdmin={effectiveIsAdmin} notifications={notifications} unread={unread} activePage="dashboard" roleAccess={effectiveRoleAccess} newHires={newHires} previewingAs={previewingAs} />
      <main className="flex-1 p-6">
        {!previewAsId && <NewHireTabBar />}
        {children}
      </main>
    </div>
  );
