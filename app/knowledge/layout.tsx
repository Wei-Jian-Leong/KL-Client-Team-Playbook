...
export default async function KnowledgeLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!dbUser?.isAdmin && !roleAccess["knowledge"]) redirect("/login");
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <AppHeader session={effectiveSession} isAdmin={effectiveIsAdmin} notifications={notifications} unread={unread} activePage="knowledge" roleAccess={effectiveRoleAccess} newHires={newHires} previewingAs={previewingAs} />
