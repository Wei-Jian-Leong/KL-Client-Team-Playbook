...
export default async function ScheduleLayout({ children }: { children: React.ReactNode }) {
  ...
  const [notifications, unread, dbUser, roleAccess] = await Promise.all([
  ...
  if (!dbUser?.isAdmin && !roleAccess["schedule"]) redirect("/knowledge");

  return (
    <div className="min-h-screen flex flex-col">
      <AppHeader session={session} isAdmin={!!dbUser?.isAdmin} notifications={notifications} unread={unread} activePage="dashboard" roleAccess={roleAccess} />
