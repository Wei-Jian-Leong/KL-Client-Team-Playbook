...
export async function translateArticleToZh(
  ...
  try {
    ...
    const titlePrompt = `Translate the following English text to Simplified Chinese. Return only the translated text, nothing else.\n\n${title}`;
    const titleResult = await model.generateContent(titlePrompt);
    const titleZh = titleResult.response.text().trim();

    let contentZh: string | null = null;
    if (contentHtml) {
      // Strip binary content that would bloat the prompt
...
${strippedHtml}`;
      ...
      // Restore stripped binary content
